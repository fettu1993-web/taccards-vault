...
export async function collectionRoutes(app: FastifyInstance) {
  ...
  app.get('/', { preHandler: authenticate }, async (req, reply) => {
    ...
    const { status, sport } = req.query as Record<string, string>
  ...
  app.post('/', { preHandler: authenticate }, async (req, reply) => {
    ...
    if (!card) return reply.status(404).send({ error: 'Carta non trovata nel catalogo' })
  ...
  app.patch('/:id', { preHandler: authenticate }, async (req, reply) => {
    ...
    if (!existing) return reply.status(404).send({ error: 'Carta non trovata nella tua collezione' })
  ...
  app.delete('/:id', { preHandler: authenticate }, async (req, reply) => {
    ...
    if (!existing) return reply.status(404).send({ error: 'Carta non trovata' })
